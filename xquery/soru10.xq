(:forex buying >3 ise <high> tagı içinde ismini yazdır değilse <low> tagı içinde yazdır:)
xquery version "1.0";
for $cur in doc("http://tcmb.gov.tr/kurlar/today.xml")/Tarih_Date/Currency
return 
if($cur/ForexBuying >3) then
<high>{data($cur/Isim)}</high>
else
<low>{data($cur/Isim)}</low>(: Stylus Studio meta-information - (c) 2004-2009. Progress Software Corporation. All rights reserved.
